...
module.exports = Helpers.withDefaults([
    {
        ...
        handler: async (request) => {
            ...
            const { cardService, userService } = request.services();
            await cardService.saveWordCard(request.payload, request.auth.credentials);
    ...
    },
    {
        ...
        handler: async (request) => {
            ...
            const { cardService, userService } = request.services();
            await cardService.updateWordCard(id, request.payload, request.auth.credentials);
            return userService.getUser(request.auth);
        }
    },
    {
        ...
        handler: async (request) => {
            ...
            const { id } = request.params;
            const { cardService, userService } = request.services();
            await cardService.removeWordCard(id, request.auth.credentials);
            return userService.getUser(request.auth);
        }
    }
]);
